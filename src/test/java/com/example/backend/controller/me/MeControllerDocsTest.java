        ...
        mockMvc.perform(patch("/api/me/profile")
                ...
                .andDo(document("me-profile-update",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("followedByMe").type(BOOLEAN).description("내가 이 사용자를 팔로우 중인지 여부(내 프로필은 항상 false)")
                        )
                ));
    }
...
@Test
void 내_리마인더_설정_조회_200(RestDocumentationContextProvider restDocumentation) throws Exception {
    MockMvc mockMvc = mockMvc(restDocumentation);
    var me = docs.saveUser("mesettings", "나");
    me.updateReminderSettings(true, true, false);
    docs.userRepository().saveAndFlush(me);
    String token = docs.issueTokenFor(me);

    mockMvc.perform(get("/api/me/reminder-settings")
                    .header(DocsTestSupport.headerName(), DocsTestSupport.auth(token)))
            .andExpect(status().isOk())
...
@Test
void 내_리마인더_설정_수정_200(RestDocumentationContextProvider restDocumentation) throws Exception {
    MockMvc mockMvc = mockMvc(restDocumentation);
    var me = docs.saveUser("mesettingspatch", "나");
    String token = docs.issueTokenFor(me);

    var req = new HashMap<String, Object>();
    req.put("pinRemindBrowserNotify", true);
    req.put("pinRemindSound", true);
    req.put("pinRemindVibrate", true);
    ...
    mockMvc.perform(patch("/api/me/reminder-settings")
                    ...
                    .content(objectMapper.writeValueAsString(req)))
            .andExpect(status().isOk())
            .andDo(document("me-reminder-settings-patch",
                    preprocessRequest(prettyPrint()),
                    preprocessResponse(prettyPrint()),
                    requestHeaders(
                            headerWithName(DocsTestSupport.headerName()).description("Bearer {accessToken}")
