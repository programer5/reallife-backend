...
==== 3-3-1) 내 읽음 상태 조회 (unread divider 계산용)

include::{snippets}/conversations-read-state-200/http-request.adoc[]
include::{snippets}/conversations-read-state-200/http-response.adoc[]

==== 3-3-2) 읽음 표시 조회 (1:1 DM - 상대 lastReadAt)

include::{snippets}/conversations-read-receipts-200/http-request.adoc[]
include::{snippets}/conversations-read-receipts-200/http-response.adoc[]
...
|`message-updated` |메시지 수정 발생 |해당 대화방 메시지 목록 갱신(또는 해당 messageId만 UI 갱신)
|`message-deleted` |메시지 삭제 발생 |해당 대화방 메시지 목록 재조회
|`pin-created` |핀 생성 |해당 대화방 핀 목록 재조회(또는 상단 pinned UI 즉시 갱신)
|`pin-updated` |핀 상태 변경/숨김(done/cancel/dismiss) |해당 대화방 핀 목록 재조회(또는 상단 pinned UI에서 해당 핀 제거/갱신)
|`notification-created` |새 알림 생성 |알림 목록 재조회 / 뱃지 카운트 갱신
|`conversation-read` |상대가 대화방 읽음 처리(/read) |내 마지막 메시지에 “읽음” 표시 갱신(또는 read-receipts 재조회)
|===
[NOTE]
====
아래 payload 예시는 **실제 SSE event 데이터(JSON)** 기준입니다.
SSE에서는 `event: <type>` 헤더로 이벤트 타입이 전달되고, 본문은 JSON 입니다.
====

===== pin-created payload 예시
...
include::{snippets}/users-exists/query-parameters.adoc[]
...
=== 프로필 조회 (userId)
include::{snippets}/users-profile-get-by-id/http-request.adoc[]
include::{snippets}/users-profile-get-by-id/http-response.adoc[]
include::{snippets}/users-profile-get-by-id/path-parameters.adoc[]
include::{snippets}/users-profile-get-by-id/response-fields.adoc[]
...
[NOTE]
====
- `name`은 선택 입력입니다. 보내면 표시 이름을 함께 수정합니다.
- 응답의 `followedByMe`는 현재 로그인 사용자가 이 프로필을 팔로우 중인지 나타냅니다.
====
...
[NOTE]
====
현재 검색 API는 Elasticsearch가 활성화되어 있으면 ES 조회를 우선 사용하고, 장애·미설정 상태에서는 DB fallback으로 자동 전환됩니다.
응답 계약과 인덱싱 훅은 Elasticsearch/OpenSearch로 확장하기 쉽게 맞춰져 있습니다.
`items[].anchorType / anchorId`를 사용하면 프론트에서 메시지·액션·캡슐 위치로 다시 진입해 하이라이트할 수 있습니다.
====

include::{snippets}/search-unified-200/http-request.adoc[]
include::{snippets}/search-unified-200/query-parameters.adoc[]
include::{snippets}/search-unified-200/http-response.adoc[]
include::{snippets}/search-unified-200/response-fields.adoc[]

=== Elasticsearch 기존 데이터 재색인
[NOTE]
====
처음 Elasticsearch를 붙이면 기존 DB 데이터는 자동으로 한 번에 들어가지 않을 수 있습니다.
`POST /api/search/admin/reindex`를 호출하면 메시지/액션/캡슐/피드 전체를 다시 인덱싱합니다.
보안을 위해 아래 조건이 모두 필요합니다.

- 인증된 사용자 access token
- 운영자(ops) allowlist 권한
- `X-Search-Reindex-Token` 헤더
====

include::{snippets}/search-reindex-200/http-request.adoc[]
include::{snippets}/search-reindex-200/query-parameters.adoc[]
include::{snippets}/search-reindex-200/http-response.adoc[]
include::{snippets}/search-reindex-200/response-fields.adoc[]
...
=== 댓글 답글 생성(201)
include::{snippets}/comments-create-reply-201/http-request.adoc[]
include::{snippets}/comments-create-reply-201/http-response.adoc[]
...
==== 인기 정렬 예시(POPULAR)
include::{snippets}/comments-list-200-popular/http-request.adoc[]
include::{snippets}/comments-list-200-popular/http-response.adoc[]
...
==== 전체공개 글 노출 예시
include::{snippets}/feed-get-public-visible/http-request.adoc[]
include::{snippets}/feed-get-public-visible/http-response.adoc[]

==== 근처 피드 조회
include::{snippets}/feed-nearby-get/http-request.adoc[]
include::{snippets}/feed-nearby-get/http-response.adoc[]

[NOTE]
====
피드 노출 규칙은 아래처럼 동작합니다.

- `ALL` : 비팔로우 사용자에게도 노출
- `FOLLOWERS` : 작성자를 팔로우한 사용자 + 작성자 본인에게 노출
- `PRIVATE` : 작성자 본인만 노출
- 근처 피드는 `lat`, `lng` 기준으로 위치 정보가 있는 게시글을 거리순으로 반환합니다. 기존 위치 없는 게시글은 거리 계산 대상이 아니거나 `distanceKm`가 `null`일 수 있습니다.
====
...
=== 메시지 수정
include::{snippets}/messages-update/http-request.adoc[]
include::{snippets}/messages-update/http-response.adoc[]
...
=== 공동 플레이 세션 생성
include::{snippets}/playback-sessions-create/http-request.adoc[]
include::{snippets}/playback-sessions-create/http-response.adoc[]

=== 공동 플레이 세션 목록
include::{snippets}/playback-sessions-list/http-request.adoc[]
include::{snippets}/playback-sessions-list/http-response.adoc[]

=== 공동 플레이 재생 상태 변경
include::{snippets}/playback-sessions-update-state/http-request.adoc[]
include::{snippets}/playback-sessions-update-state/http-response.adoc[]

=== 공동 플레이 세션 종료
include::{snippets}/playback-sessions-end/http-request.adoc[]
include::{snippets}/playback-sessions-end/http-response.adoc[]

=== AI 추천 답장 생성
include::{snippets}/conversation-ai-reply/http-request.adoc[]
include::{snippets}/conversation-ai-reply/http-response.adoc[]

=== AI 액션 실행 - 일정 핀 생성
include::{snippets}/conversation-ai-action-execute/http-request.adoc[]
include::{snippets}/conversation-ai-action-execute/http-response.adoc[]

=== AI 액션 실행 - 이따 알림 자동 보정
include::{snippets}/conversation-ai-action-execute-reminder/http-request.adoc[]
include::{snippets}/conversation-ai-action-execute-reminder/http-response.adoc[]

=== AI 액션 실행 - 지도 URL 반환
include::{snippets}/conversation-ai-action-execute-map/http-request.adoc[]
include::{snippets}/conversation-ai-action-execute-map/http-response.adoc[]

[NOTE]
====
AI 추천 답장 API는 `OPENAI_API_KEY`가 있으면 OpenAI 호출 결과를 사용하고, 키가 없거나 호출에 실패하면 rule 기반 fallback을 반환합니다.
액션 실행 API는 `schedule`, `reminder`, `notify` 타입에서 대화 핀을 생성하고 `targetUrl`로 후속 이동 경로를 제공합니다. `reminder`/`notify`는 `startAt`이 없어도 "이따", "나중", "내일", "오전/오후 n시" 같은 문장을 기준으로 시간을 보정합니다. `map` 타입은 지도 검색 URL을 반환합니다.
====
...
=== 핀 완료
include::{snippets}/pins-done/http-request.adoc[]
include::{snippets}/pins-done/http-response.adoc[]

=== 핀 취소
include::{snippets}/pins-cancel/http-request.adoc[]
include::{snippets}/pins-cancel/http-response.adoc[]

=== 핀 숨김(개인)
include::{snippets}/pins-dismiss/http-request.adoc[]
include::{snippets}/pins-dismiss/http-response.adoc[]

=== 핀 단건 조회
include::{snippets}/pins-get/http-request.adoc[]
include::{snippets}/pins-get/http-response.adoc[]

=== 핀 수정
include::{snippets}/pins-update/http-request.adoc[]
include::{snippets}/pins-update/http-response.adoc[]

=== 핀 삭제
include::{snippets}/pins-delete/http-request.adoc[]
include::{snippets}/pins-delete/http-response.adoc[]
...
[NOTE]
====
알림 목록은 서버에서 `priorityScore DESC, createdAt DESC` 기준으로 정렬합니다.
응답에는 Inbox UX를 위한 `category`, `priorityScore`, `targetPath`, `targetLabel`, `actionHint`가 포함됩니다.
====
...
- `POST_LIKE` : 게시글 좋아요
- `PIN_CREATED` : 액션/핀 생성
- `PIN_REMIND` : 약속/할일 리마인드 도착

`PIN_REMIND`는 별도 API를 호출해서 만드는 것이 아니라, 서버 스케줄러가 `conversation_pins.remind_at`을 보고 자동으로 생성합니다.
====
...
==== 댓글 좋아요 등록 (204)
include::{snippets}/comment-likes-post-204/http-request.adoc[]
include::{snippets}/comment-likes-post-204/path-parameters.adoc[]
include::{snippets}/comment-likes-post-204/request-headers.adoc[]
include::{snippets}/comment-likes-post-204/http-response.adoc[]

==== 댓글 좋아요 취소 (204)
include::{snippets}/comment-likes-delete-204/http-request.adoc[]
include::{snippets}/comment-likes-delete-204/path-parameters.adoc[]
include::{snippets}/comment-likes-delete-204/request-headers.adoc[]
include::{snippets}/comment-likes-delete-204/http-response.adoc[]
...
====
운영 상태 점검용 엔드포인트입니다.
- 일반 사용자 API가 아니라 운영/관리 용도입니다.
- 인증 후 접근해야 합니다.
====

=== 운영 상태 요약 조회
include::{snippets}/admin-health-get/http-request.adoc[]
include::{snippets}/admin-health-get/http-response.adoc[]
include::{snippets}/admin-health-get/request-headers.adoc[]
include::{snippets}/admin-health-get/response-fields.adoc[]

=== 실시간 상태 조회
include::{snippets}/admin-health-realtime-get/http-request.adoc[]
include::{snippets}/admin-health-realtime-get/http-response.adoc[]
include::{snippets}/admin-health-realtime-get/request-headers.adoc[]
include::{snippets}/admin-health-realtime-get/response-fields.adoc[]

=== Reminder 상태 조회
include::{snippets}/admin-health-reminder-get/http-request.adoc[]
include::{snippets}/admin-health-reminder-get/http-response.adoc[]
include::{snippets}/admin-health-reminder-get/request-headers.adoc[]
include::{snippets}/admin-health-reminder-get/response-fields.adoc[]

=== Admin Dashboard 조회
include::{snippets}/admin-dashboard-get/http-request.adoc[]
include::{snippets}/admin-dashboard-get/http-response.adoc[]
include::{snippets}/admin-dashboard-get/request-headers.adoc[]
include::{snippets}/admin-dashboard-get/response-fields.adoc[]

=== 최근 서버 에러 조회
include::{snippets}/admin-errors-get/http-request.adoc[]
include::{snippets}/admin-errors-get/http-response.adoc[]
include::{snippets}/admin-errors-get/request-headers.adoc[]
include::{snippets}/admin-errors-get/response-fields.adoc[]

=== Slack 테스트 알림 전송
include::{snippets}/admin-alert-test-post/http-request.adoc[]
include::{snippets}/admin-alert-test-post/http-response.adoc[]
include::{snippets}/admin-alert-test-post/request-headers.adoc[]
include::{snippets}/admin-alert-test-post/response-fields.adoc[]

=== 최근 운영 알림 이력 조회
include::{snippets}/admin-alert-history-get/http-request.adoc[]
include::{snippets}/admin-alert-history-get/http-response.adoc[]
include::{snippets}/admin-alert-history-get/request-headers.adoc[]
include::{snippets}/admin-alert-history-get/response-fields.adoc[]
...
=== 대화방 타임 캡슐 생성
include::{snippets}/capsules-create/http-request.adoc[]
include::{snippets}/capsules-create/http-response.adoc[]

=== 대화방 타임 캡슐 목록 조회
include::{snippets}/capsules-list/http-request.adoc[]
include::{snippets}/capsules-list/http-response.adoc[]

=== 타임 캡슐 열기
include::{snippets}/capsules-open/http-request.adoc[]
include::{snippets}/capsules-open/http-response.adoc[]

=== 타임 캡슐 수정
include::{snippets}/capsules-update/http-request.adoc[]
include::{snippets}/capsules-update/http-response.adoc[]

=== 타임 캡슐 삭제
include::{snippets}/capsules-delete/http-request.adoc[]
include::{snippets}/capsules-delete/http-response.adoc[]

=== Post Media 응답 가이드
`imageUrls` 는 하위 호환용 필드이고, 권장 필드는 `mediaItems` 입니다.
...
=== 로그인 식별자(identifier)
로그인은 이제 `email` 고정이 아니라 `identifier` 필드를 사용합니다.

`identifier` 에는 다음 둘 다 넣을 수 있습니다.
- 이메일
- handle(아이디)
...
include::{snippets}/home-today-widget-get/http-request.adoc[]
include::{snippets}/home-today-widget-get/http-response.adoc[]
include::{snippets}/home-today-widget-get/response-fields.adoc[]

== 홈 Reminder Summary

[NOTE]
====
이제 Reminder Summary는 클라이언트 동기화 값이 아니라 서버에 저장된 사용자 Reminder 설정을 기준으로 내려갑니다.
`settingsSource`는 `SERVER`이며, `browserNotifyEnabled`, `soundEnabled`, `vibrateEnabled`를 함께 제공합니다.
====

include::{snippets}/home-reminder-summary-get/http-request.adoc[]
include::{snippets}/home-reminder-summary-get/http-response.adoc[]
include::{snippets}/home-reminder-summary-get/response-fields.adoc[]

[NOTE]
====
이 API는 Home 상단의 "오늘 예정 액션" 위젯 전용 응답입니다.
프론트가 My Activity 전체 목록을 내려받아 다시 필터링하지 않도록, 오늘 일정과 요약 통계를 한 번에 제공합니다.
====

== 내 활동 보관함 (My Activity)

=== 내 액션 목록 조회
include::{snippets}/me-activity-actions-get/http-request.adoc[]
include::{snippets}/me-activity-actions-get/http-response.adoc[]

[NOTE]
====
내 액션 목록의 `conversationId`, `sourceMessageId`를 이용하면 프론트에서 원본 대화/메시지로 바로 이동하는 Deep Link를 구성할 수 있습니다.
====

=== 내 캡슐 목록 조회
include::{snippets}/me-activity-capsules-get/http-request.adoc[]
include::{snippets}/me-activity-capsules-get/http-response.adoc[]

[NOTE]
====
내 캡슐 목록의 `conversationId`, `messageId`를 이용하면 프론트에서 원본 대화의 해당 메시지 위치로 이동하는 Deep Link를 구성할 수 있습니다.
====

=== 내 공유 목록 조회
include::{snippets}/me-activity-shares-get/http-request.adoc[]
include::{snippets}/me-activity-shares-get/http-response.adoc[]
